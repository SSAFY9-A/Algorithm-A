...
/*
	...
	어려웠떤 부분은 [내현재돈], [다음달에 오를 주식 종목] 들을 가지고
	어떤 종목을 몇개씩 사야 다음달에 최대의 이익인지를 찾는 부분을 구현하는데 시간이 많이 걸려서 실패함.

	두번째로 풀땐 앞서서 어려웠던 부분을 구현해서 성공.

	사용한 방식은

	1. 0월 ~ L월 까지 시뮬레이션 간다. L월되면 전부 매도후 시뮬 종료.
	2. 현재 월에서 다음월에 오를 종목들 찾기.
	3. 현재 가진 돈으로 그 종목들을 각각 몇 개씩 사야 다음달 전량 매도에서 최대의 이익을 볼 수 있는지 찾기(dfs).
	4. 그 조합으로 다 사고 다음달로 넘어가.
*/
